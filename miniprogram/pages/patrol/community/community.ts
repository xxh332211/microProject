...
Page({
  ...
  selectSearch (e:any) {
    ...
    api.getCommitte([selected.committee_id]).then((res:any)=>{
      this.setData({
  ...
  search (e:any) {
    console.log(e);
    api.searchComunity(e.detail.value).then((res:any)=>{
      console.log(res)
      this.setData({
  ...
  getPenalties () {
    api.getComunityPenalties(this.data.s_community.subdistrict_id).then((res:any)=>{
      this.setData({
  ...
  selectComunitee (e:any) {
    ...
    api.getComunity([this.data.s_committee.committee_id]).then((res:any)=>{
      this.setData({
  ...
  upDecut (e:any){
    ...
    api.upDecutComunity(_data).then((res:any)=>{
  ...
  historyInit () {
    ...
    api.getCurrentPenalties('community').then((res:any)=>{
      if (res) {
        ...
        // 没有缓存数据
        api.getCommitte([this.data.record.committee_id]).then((res:any)=>{
          this.setData({
            s_committee:  res.data.result[0]
          })
        })
        api.getComunity([this.data.record.committee_id+''],this.data.record.subdistrict_id).then((res:any)=>{
          this.setData({
